...
cd lib
if [ -e /usr/local/lib/antlr-4.5.1-complete.jar ]
then
  ln -s /usr/local/lib/antlr-4.5.1-complete.jar antlr.jar
else
  curl --silent -L http://www.antlr.org/download/antlr-4.5.1-complete.jar >antlr.jar
fi
cd ..
...
then
  ...
  curl --silent -L https://github.com/jboss-javassist/javassist/releases/download/rel_3_20_0_ga/javassist.jar >javassist.jar
fi
cd ..
...
echo TS_ROOT=\"`pwd`\" >>build/bin/grun
